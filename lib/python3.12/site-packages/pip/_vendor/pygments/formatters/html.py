...
class HtmlFormatter(Formatter):
    r"""
    Format tokens as HTML 4 ``<span>`` tags. By defaults, the content is enclosed
    ...
    `nowrap`
        ...
        around the tokens. This disables most other options (defaults: ``False``).

    `full`
        Tells the formatter to output a "full" document, i.e. a complete
        self-contained document (defaults: ``False``).
    ...
    `style`
        The style to use, can be a string or a Style subclass (defaults:
        ``'defaults'``). This option has no effect if the `cssfile`
    ...
    `cssclass`
        CSS class for the wrapping ``<div>`` tag (defaults: ``'highlight'``).
        If you set this option, the defaults selector for `get_style_defs()`
        will be this class.

        .. versionadded:: 0.9
           If you select the ``'table'`` line numbers, the wrapping table will
           have a CSS class of this string plus ``'table'``, the defaults is
           accordingly ``'highlighttable'``.

    `cssstyles`
        Inline CSS styles for the wrapping ``<div>`` tag (defaults: ``''``).

    `prestyles`
        Inline CSS styles for the ``<pre>`` tag (defaults: ``''``).
    ...
    `linenos`
        ...
        The defaults value is ``False``, which means no line numbers at all.

        **Note:** with the defaults ("table") line number mechanism, the line
        numbers and code can have different line heights in Internet Explorer
        unless you give the enclosing ``<pre>`` tags an explicit ``line-height``
        CSS property (you get the defaults line spacing with ``line-height:
        125%``).
    ...
    `linenostart`
        The line number for the first line (defaults: ``1``).
    ...
    `linenospecial`
        ...
        class ``"special"`` (defaults: ``0``).

    `nobackground`
        If set to ``True``, the formatter won't output the background color
        for the wrapping element (this automatically defaults to ``False``
        when there is no wrapping element [eg: no argument for the
        `get_syntax_defs` method given]) (defaults: ``False``).
    ...
    **Subclassing the HTML formatter**
    ...
    The defaults `wrap()` implementation adds a ``<div>`` and a ``<pre>`` tag.
